  ...
  try {
      // Validate quantities before proceeding
        foreach ($cart_items as $item) {
            ...
            $stmt_stock = $conn->prepare("SELECT SUM(quantity) AS total_stock FROM stockin2 WHERE pro_code= ? AND quantity > 0");
        ...
        if($sales_method == "Cash"){
            ...
            $query = "SELECT *
                        FROM stockin2 WHERE pro_code = ? AND quantity > 0 ORDER BY stockin_date ASC";
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                $stmt = $conn->prepare("
                        SELECT 
                            SUM(quantity) AS total_stockin FROM stockin2 WHERE pro_code = $product_id AND quantity > 0
                    ");
                    $stmt->execute();
                ...
                if ($remainingQty >= $batchQty) {
                    ...
                    $updateQuery = "UPDATE stockin2 SET quantity = 0, total = 0, subtotal = 0, vat = 0 WHERE id = ?";
                    ...
                    $updateQuery = "UPDATE stockin2 SET quantity = ?, total = ?, subtotal = ?, vat = ?  WHERE id = ?";
                    $updateStmt = $conn->prepare($updateQuery);
            ...
            $stockOutStmt = $conn->prepare("INSERT INTO stockout (transac_id, pro_code, pro_name, category, quantity, unit, sale_price, subtotal, vat, total_price_reduced,  total, profit, stockout_date, expired_date, status, employee,  customer_name, store) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW(), ?, ?, ?, 'Store 2')");
            ...
            $stockOutStmt = $conn->prepare("INSERT INTO sales (transac_id, PRODUCT_CODE, NAME, CATEGORY, QUANTITY, UNIT, PRICE, SUBTOTAL, VAT, TOTAL, total_price_reduced, profit, DATE, expired_date, STATUS, EMPLOYEE, CUSTOMER_NAME, store) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW(), ?, ?, ?, 'Store 2')");
...
<body>
    ...
    <a href="sales2.php"><button type="submit">Back</button></a>
</body>
</html>
